...
SOURCE_FASTA_BASE="https://hgdownload.soe.ucsc.edu/goldenPath/hg19/bigZips"
...
curl --fail --location --retry 3 -o "${tmp_dir}/hg19.fa.gz" \
  "${SOURCE_FASTA_BASE}/hg19.fa.gz"
curl --fail --location --retry 3 -o "${tmp_dir}/source_fasta.md5sum.txt" \
  "${SOURCE_FASTA_BASE}/md5sum.txt"
...
(
  ...
  awk '$2 == "hg19.fa.gz" {print}' source_fasta.md5sum.txt > source_fasta.selected.md5
  test -s chain.selected.md5 && test -s fasta.selected.md5 && test -s source_fasta.selected.md5
  md5sum --check chain.selected.md5
  md5sum --check fasta.selected.md5
  md5sum --check source_fasta.selected.md5
)
...
gzip -dc "${tmp_dir}/hg19.fa.gz" > "${REFERENCE_DIR}/GRCh37.fa"
...
source_reference_url=${SOURCE_FASTA_BASE}/hg19.fa.gz
source_reference_md5_url=${SOURCE_FASTA_BASE}/md5sum.txt
downloaded_utc=$(date -u +%Y-%m-%dT%H:%M:%SZ)
EOF

echo "[OK] ${REFERENCE_DIR}/grch37_to_grch38.chain"
echo "[OK] ${REFERENCE_DIR}/GRCh38.fa"
echo "[OK] ${REFERENCE_DIR}/GRCh37.fa"
